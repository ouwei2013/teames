...
def test_list_authenticated_providers_current_endpoint_marks_named_custom_current(monkeypatch):
    """When current_base_url matches one custom endpoint, mark it current
    without collapsing its slug to bare ``custom``."""
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr(providers_mod, "HERMES_OVERLAYS", {})
